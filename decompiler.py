...
# 从jar反编译出java文件
def decompiler_jar():
    cmd_txt = "java -cp {} org.jetbrains.java.decompiler.main.decompiler.ConsoleDecompiler -dgs=true {} {}".format(
    ...
    with zipfile.ZipFile(Path(output_path).joinpath(getJarName(jar_path)), mode='r') as zfile:
        for file in zfile.namelist():
            zfile.extract(file, Path(output_path).joinpath(
                getJarName(jar_path).rstrip('.jar') + "_tmp"))
...
def gen():
    ...
    src_path = Path(output_path).joinpath(
        getJarName(jar_path).rstrip('.jar') + "_tmp")
    src = src_path.joinpath("BOOT-INF/classes/com")
    des = res.joinpath("src/main/java/com")
    shutil.copytree(src, des)

    shutil.move(src_path.joinpath("BOOT-INF/classes/application.properties"),
